...
5. **Income.** ACS median household income (B19013) by tract for Florida counties that have a parcel extract, and by block group for Orange County. The value is not copied onto every tile. `/api/parcels` joins it from `data/fixtures/income-tracts.geojson` (and the Orange block-group fixture) at query time. The same tract minimum also filters eligible-tract overlays whose GEOID is in that fixture. Acreage and AADT are not on tract features, so those sliders stay parcel-only. Include unknown income keeps parcels and tracts that have no joined median.
6. **AADT.** Nearest FDOT count segment within 15 km, from the statewide sidecar `data/fixtures/aadt-segments.geojson`. The map overlay is still only segments at or above 15,000. Include unknown AADT keeps parcels with no segment in range. Outside Florida both fields stay unknown.

Acreage, income, AADT, land use, and Opportunity Zone filters run in the parcel query before the viewport is thinned, then again on the loaded features so the map and ranked list stay in step. `/api/parcels?filter=1` is what the map calls (`minAcres`, `minIncome`, `minAadt`, `landUse`, `oz`, …). Florida parcels in that response pick up ACS median income and the nearest FDOT AADT from the sidecars before those filters run. Other states leave those fields unknown; uncheck “include unknown” to hide them. The map hides non-matches with a single `filterMatch` flag rather than a list of parcel ids.
...
| Median household income | ACS 5-year B19013 via [Census Reporter](https://api.censusreporter.org) (fixtures). Live Census Bureau API is optional. | Florida parcel counties; block groups remain Orange (`12095`) |
...
| AADT | FDOT Annual Average Daily Traffic (Historical TDA), year 2025 | [AGOL FeatureServer](https://services1.arcgis.com/O1JpcwDW8sjYuddV/arcgis/rest/services/Annual_Average_Daily_Traffic_Historical_TDA/FeatureServer/0) |
