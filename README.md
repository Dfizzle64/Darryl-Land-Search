...
**Site screening** (off until you turn a layer on): FEMA flood zones (with a base flood elevation only when NFHL publishes one, plus the community id and no invented CRS class), National Wetlands Inventory, public school ratings, Orange County, Charlotte-Mecklenburg, and Cobb County attendance zones, and Orange County water, sewer, and electric service areas. Charlotte Water has no public polygon, so the drawer uses the city boundary only as a jurisdiction proxy and leaves unincorporated Mecklenburg unverified. Forty Cobb County parcels in the Atlanta fixture join CCSD or Marietta schools (GOSA 2025 CCRPI scores, not A–F letters), NFHL flood zones, and water, sewer, and electric. Gas stays unknown. Five of those Cobb parcels stay sewer gaps. Outside Orange County, electric falls back to HIFLD retail territories (Duke Energy Carolinas and EnergyUnited EMC in Mecklenburg; the Cobb batch joins one HIFLD name when territories overlap). The parcel drawer adds the mailing address already on the extract plus the county appraiser and, for entity owners, the state business search. It does not add phone numbers or emails. Coverage and refresh steps: [`docs/screening-layers.md`](docs/screening-layers.md).
