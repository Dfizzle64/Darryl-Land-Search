...
- The seven primary markets draw rural-eligible tracts (orange) and urban eligible tracts (blue). Fourteen smaller MSAs under Other do the same. 90-minute sheds are approximate county rings, not isochrones. Other markets load 5–150 acre parcel tiles where a public polygon source is wired (`docs/market-parcels.md`). Charleston MSA uses Charleston County ENERGOV ent, Berkeley County Addr_muni, and Dorchester Parcels_Public. Mount Pleasant town zoning and future land use overlay the Charleston County parcels. Folly Beach stays a third-party gap. Isle of Palms, Sullivan's Island, and James Island stay gaps. Token-walled services (Charleston Parcel_Search / Public_Search, Berkeley AGOL parcels) are not ingested. Inside the Orlando shed, Brevard, Marion, Sumter, and Volusia are thinner samples. South Carolina tracts add a governor-filed note (list not public, not designated) where that caveat is in the notes. They are not certified 2027 QOZs. The SC multifamily shortlist is a priority filter on the primary rural layer, not a nomination.
