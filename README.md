...
- Jefferson County, Alabama unincorporated zoning is `DDS/FutureLandUse` FeatureServer/4 `ZNCODE`, and only where a parcel had no city code. These Alabama counties still have no county zoning layer on the live shelves: Autauga, Blount, Elmore, Escambia, Limestone, Marshall, Mobile, Montgomery, Morgan, Tuscaloosa, Walker, and Washington. City layers in those counties are unchanged.
- Hinds County, Mississippi has no parcel shelf and no highway zoning feature service. The county zoning map is a paper atlas, so no zoning codes were invented. James Island, South Carolina publishes PDF maps only; those districts stay blank (`docs/muni-overlay-consolidator.md`).
