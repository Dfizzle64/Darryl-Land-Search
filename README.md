...
npm run seed:parcels:tampa    # Hillsborough, Pasco, Pinellas, and the Polk upgrade
...
Other MSAs use the same tile grid and the same zoom / area-lock / Show parcels gate. They load only while that market is selected. Refresh and the complete-vs-sample-vs-gap table: [`docs/market-parcels.md`](docs/market-parcels.md). Tampa Hillsborough, Pasco, Pinellas, and Polk use county GIS cards, including city zoning overlays and the gaps those cards could not fill: [`docs/tampa-shed-parcels.md`](docs/tampa-shed-parcels.md).
