...
**Site screening** (off until you turn a layer on): FEMA flood zones (with a base flood elevation only when NFHL publishes one, plus the community id and no invented CRS class), National Wetlands Inventory, public school ratings, Orange County, Charlotte-Mecklenburg, Cobb County, and DeKalb County attendance zones, and Orange County water, sewer, and electric service areas. Charlotte Water has no public polygon, so the drawer uses the city boundary only as a jurisdiction proxy and leaves unincorporated Mecklenburg unverified. Forty Cobb County parcels in the Atlanta fixture join CCSD or Marietta schools (GOSA 2025 CCRPI scores, not A–F letters), NFHL flood zones, and water, sewer, and electric. Gas stays unknown. Five of those Cobb parcels stay sewer gaps. Forty DeKalb County parcels join DCSD schools the same way. Two of those sit in the City of Atlanta and still zone to Druid Hills DCSD; their water and sewer name Atlanta DWM as a jurisdiction proxy. The other 38 name DeKalb DWM the same way, because DeKalb publishes no service polygon. Outside Orange County, electric falls back to HIFLD retail territories (Duke Energy Carolinas and EnergyUnited EMC in Mecklenburg; the Cobb batch joins one HIFLD name when territories overlap; the DeKalb batch joins the cooperative when Georgia Power overlaps Snapping Shoals EMC or Walton EMC). The parcel drawer adds the mailing address already on the extract plus the county appraiser and, for entity owners, the state business search. It does not add phone numbers or emails. Coverage and refresh steps: [`docs/screening-layers.md`](docs/screening-layers.md).
...
5. **Income.** ACS 5-year 2020–2024 median household income (B19013_001E, 2024 inflation-adjusted dollars) by census tract for every state with a live parcel extract, and by block group for Orange County. The value is not copied onto every tile. `/api/parcels` joins it from `data/fixtures/acs-b19013-tracts.json` (GEOID) and `data/fixtures/income-tracts.geojson` (centroid fallback). The same tract minimum also filters eligible-tract overlays whose GEOID is in that table. Acreage and AADT are not on tract features, so those sliders stay parcel-only. A minimum excludes parcels and tracts with no joined median unless Include unknown income is on.
6. **AADT.** Nearest count within 15 km. Florida uses FDOT 2025 segments in `data/fixtures/aadt-segments.geojson` for every footprint county. North Carolina, Georgia, South Carolina, Tennessee, Mississippi, Alabama, and Arkansas use `data/fixtures/aadt-state-dots.geojson` (published counts, not estimated) for footprint counties the layer actually covers. North Carolina gap counties use NCDOT 2024 stations, field `AADT_2024` (a string; blanks dropped). Durham, Mecklenburg, and Wake stay on `AADT_2022`. South Carolina is SCDOT 2025 points, field `FactoredAA`. Mississippi is the HDR AGOL republish of RCI layer `RC_AADT_2019`, field `ADT_21` (through 2021), not an MDOT-hosted service. Georgia is GDOT stations republished by DeKalb County GIS, field `aadt`, with no year on the layer. Alabama is ALDOT `AADT` for 2024. The map overlay is still only Florida segments at or above 15,000. Include unknown AADT keeps parcels with no count in range. Outside Florida the drawer says Nearest AADT, not FDOT. A county with no published count stays unknown.

Acreage, income, AADT, land use, and Opportunity Zone filters run in the parcel query before the viewport is thinned, then again on the loaded features so the map and ranked list stay in step. `/api/parcels?filter=1` is what the map calls (`minAcres`, `minIncome`, `minAadt`, `landUse`, `oz`, …). Parcels in that response pick up ACS tract median income and the nearest published traffic count before those filters run. Florida counts are FDOT. Other joined counts are the state DOT layer for that footprint county. A set income minimum drops parcels with no joined median unless Include unknown income is on. The map hides non-matches with a single `filterMatch` flag rather than a list of parcel ids.

Attributes were not dropped to fit Git or Vercel. Owner, sale, tax, mailing, and Orange zoning stay on the parcel tiles. Income and AADT stay in sidecar files and are joined when the map asks for a viewport, so the same census number is not repeated on every parcel. Refresh with `npm run seed:signals`. Parcel rings for Orange come from the property appraiser cadastre. Other counties still use their public GIS polygons, simplified at about 1–7 meters. An earlier pass collapsed dense shorelines to a handful of vertices and treated Esri holes as separate polygons.
...
South Carolina’s governor submitted OZ 2.0 nominations, announced September 23, 2026. The source list is the [SC Commerce Final Recommendations PDF dated September 22, 2026](https://s3.amazonaws.com/cdn.sccommerce.com/scdoc-public/2026-09/Final%20Recommendations%20-%202026%209.22.26.pdf) (112 GEOIDs), committed at `data/oz/sc-oz2-nominated-official-sccommerce-2026-09-23.csv`. In South Carolina the map, tract list, and tract filters show **only** those GEOIDs, labeled **Governor-nominated / awaiting Treasury**. Eligible tracts that were not nominated are not shown. Other states still show the federal eligible list as **Eligible — not designated** (rural primary markets: **Eligible (rural) — not designated**). Nothing on the map is a certified 2027 QOZ, and nomination alone is not a tax benefit. Eligible ≠ nominated ≠ certified. Charleston, Columbia, and Greenville are nominated-only. Charlotte’s South Carolina fringe (York, Lancaster, and Chester) and Savannah’s South Carolina fringe (Beaufort and Jasper) drop eligible-only South Carolina tracts; North Carolina and Georgia tracts in those markets stay on the eligible list. SC MF priority stays a separate internal hunt, drawn in South Carolina only where the tract is also on the nominated list.
...
Tier A is a gold outline and pin. Tier B is blue. Both sit on top of the rural fill. The drawer shows the corridor label, tier, multifamily rationale, acreage realism, and notes. Nom-watch in those notes is an inference, not a confirmed nomination. The shortlist itself is not a nominated list. In South Carolina a shortlist tract is drawn only when its GEOID is also on the official 112, and then it reads **Governor-nominated / awaiting Treasury**. Shortlist GEOIDs that were not nominated are not shown. This shortlist does not change Atlanta, Tampa, Orlando, Nashville, Raleigh-Durham, or the Orange County parcel pilot.
...
| Median household income | ACS 5-year 2020–2024 B19013_001E (2024 dollars). Fixture from the Census table-based summary file; live API is `https://api.census.gov/data/2024/acs/acs5` when `CENSUS_API_KEY` is set. | Every state with a live parcel extract. Block groups remain Orange (`12095`). Charlotte NPA income is not this field. |
...
| AADT | Florida: FDOT RCI `AADT` for 2025, every footprint county. NC, GA, SC, TN, MS, AL, and AR: that state's published layer for footprint counties it actually covers. | FDOT RCI FeatureServer/0 `AADT` 2025; NCDOT `AADT_2024` (Durham, Mecklenburg, and Wake stay `AADT_2022`); GDOT stations via DeKalb GIS `aadt` (year unknown); SCDOT `FactoredAA` 2025; TDOT `AADT` 2025; Mississippi RCI `ADT_21`; ALDOT TDM `AADT` 2024; ARDOT ADT Linear `MostRecentADT` 2025 (actual stations) |
...
- Brevard city zoning and future land use are joined for Melbourne, West Melbourne, Rockledge, Satellite Beach, and Cocoa (`docs/brevard-municipal.md`). Indian Harbour Beach is zoning only. Cocoa Beach is an unofficial 2021 layer. Palm Bay and Titusville are not re-applied in this pass. County zoning is not copied into those cities. Other municipalities besides these and Orlando often have no joined FLU (Winter Park, Ocoee, Winter Garden, Apopka, Maitland, and the Brevard gap cities). FLU-only and rezoning-candidate modes omit those parcels rather than guess.
- Manatee and Sarasota city zoning is joined for Bradenton, Palmetto, Longboat Key, the City of Sarasota, North Port, and Venice (`docs/muni-overlay-consolidator.md`). North Port and Venice future land use stay blank. Anna Maria, Bradenton Beach, and Holmes Beach have no city service. County zoning is not copied into those cities.
- Punta Gorda zoning and future land use are cataloged from the city layers (`docs/muni-overlay-consolidator.md`). Charlotte County, Florida has no parcel shelf in this checkout, so nothing was stamped. County `ZONE_` `CITY` and `NEWLU` `City` stubs are not stored.
- Panhandle city zoning is joined on the existing Pensacola-market DOH shelves (`docs/muni-overlay-consolidator.md`). Pensacola, Milton, and Jay future land use stay blank. Freeport numeric codes stay blank. Bay County has no parcel shelf, so Panama City, Callaway, Mexico Beach, Lynn Haven, Parker, and Springfield are indexed only. Crestview, Niceville, Valparaiso, Mary Esther, Laurel Hill, Shalimar, Cinco Bayou, and Century stay blank.
...
- AADT is the nearest published count within 15 km, not a local-road estimate. Florida uses FDOT RCI FeatureServer/0, field `AADT`, `YEAR_=2025`, for every footprint county. The service is EPSG:26917 and the fixture is requested in WGS84. North Carolina gap counties use the 2024 AADT stations layer, field `AADT_2024` (string; blanks dropped), with the county name in title case. Durham, Mecklenburg, and Wake stay on `AADT_2022`. South Carolina uses SCDOT 2025 Statewide Traffic Points, field `FactoredAA`, including the counties that were already live. Mississippi uses the HDR AGOL republish of RCI `RC_AADT_2019`, field `ADT_21` (through 2021). `COUNTYNMBR` is the alphabetical county number, not FIPS. The MDOT 2025 app has no public FeatureServer. Georgia uses the DeKalb County GIS republish of GDOT stations (`GDOT_AADT` FeatureServer/1), field `aadt`. That layer has no year, so the year is unknown. ITOS MapServer/21 is not used. Alabama uses ALDOT TDM TrafficCounterPoint, field `AADT`, `YearAADT=2024` (2025 values are null). `LUCountyID` is the alphabetical county index, not FIPS. Tennessee uses TDOT Traffic Lines, field `AADT`, `AADTYEAR` 2025. Arkansas uses ARDOT ADT Linear, field `MostRecentADT`, `Year_ADT` 2025, actual stations only. Outside Florida the drawer says Nearest AADT. This traffic join is separate from the South Carolina OZ 2.0 nomination work.
- Charlotte / Mecklenburg neighborhood quality-of-life income (`Val_Norm` on QOL MapServer/17) is not joined. Tract median household income stays ACS B19013 everywhere, including Mecklenburg.
- The zoning card scan is 112 usable, 12 partial, 58 gap, and 1 blocked. No card folder was in this checkout, so no extra zoning URL was added beyond Jefferson County, Alabama.
- Jefferson County, Alabama unincorporated zoning is already joined from `https://jccgis.jccal.org/server/rest/services/DDS/FutureLandUse/FeatureServer/4` field `ZNCODE` (10,670 polygons), and only where a parcel had no city code. Optional future-land-use layer `/2` was not joined. These Alabama counties still have no county zoning layer: Mobile, Elmore, Autauga, Limestone, Morgan, Tuscaloosa, and Montgomery. City layers already on those shelves are unchanged. Marshall and Blount stay gaps. Walker, Washington, and Escambia stay parcel-first gaps. Decatur MapGeo and Wetumpka’s token service were not queried.
- Hinds County, Mississippi unincorporated zoning is the Raymond Courthouse paper atlas. There is no public FeatureServer, and no zoning code was invented. This pull does not add a Hinds parcel shelf. A later parcel pull would use MARIS countywide, with zoning only for the City of Jackson, Clinton, and Terry. The CMPDD Highway 80 corridor feature service is the wrong geography and is not wired. James Island, South Carolina publishes PDF maps only. County External/ENERGOV is a municipal overlay, not the ZLDR zoning layer, and is not used as James Island zoning (`docs/muni-overlay-consolidator.md`).
- Tennessee zoning cards are **9 usable / 7 partial / 0 gap**. Markets are real MSAs. The YAML package was not in this checkout (`/workspace/gis-research/cards/TN/` is absent), so no new Tennessee zoning or future-land-use URL was copied and no parcel shelf was opened. Usable FIPS already live, with the zoning joined at ingest left in place: 47009 Blount, 47021 Cheatham, 47065 Hamilton, 47093 Knox, 47113 Madison, 47119 Maury, 47125 Montgomery, 47149 Rutherford, 47187 Williamson. Partial FIPS already live: 47037 Davidson, 47043 Dickson, 47147 Robertson, 47157 Shelby, 47165 Sumner, 47189 Wilson. The packaging list labels 47107 as Bradley. Census and this shelf use 47011 for Bradley County and 47107 for McMinn County. Bradley’s live shelf is 47011 (Cleveland zoning inside the city only). 47107 has no parcel shelf and was not created. Davidson AADT stays on TDOT `Traffic_Lines/FeatureServer/0`, field `AADT`, `AADTYEAR` 2025. Bradley remains Census FIPS 47011 (`COUNTY_NUMBER` 06), not 47107.
