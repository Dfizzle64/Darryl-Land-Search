...
npm run seed:parcels:orlando  # full 5–150 acre parcels for Lake, Orange, Osceola, Polk, Seminole, Sumter
...
Choose **Orlando** for shed parcels. Lake, Orange, Osceola, Polk, Seminole, and Sumter are the complete 5–150 acre extracts. Lake uses the county open-data tax parcels. Osceola uses the county parcel layer, with St. Cloud zoning and future land use overriding a matching PIN. Seminole keeps DOH parcels and joins county land use, then Altamonte Springs and Oviedo. Sumter uses the SWFWMD parcel mirror. Orange still has the amber non-rural eligible tracts, the designated QOZ overlay, and green parcel fills. GEOID `12095016605` is still the only Orange County rural-eligible tract. Brevard, Marion, and Volusia stay thinner samples. Marion’s county parcel, FLU, and zoning services are public and are not in this extract yet. The other six metros do **not** have parcel extracts: the map draws the 2020 tract polygon and a Census Gazetteer pin. That is a coverage gap, not an empty county.
...
- Orange is the OCPA cadastre. Lake is the county open-data tax-parcel layer. Osceola is the county Parcels FeatureServer. Sumter is the SWFWMD parcel mirror. Polk and Seminole parcel geometry are every DOH parcel from 5.0 through 150.0 acres. Parcels under 5 or over 150 are omitted on purpose. Brevard, Marion, and Volusia are still windowed samples and are not capped at 150. The legacy `parcels.geojson` pilot is still a 462-parcel Orange subset.
- Lake, Osceola, Seminole, and Sumter zoning and future land use are centroid joins. St. Cloud, Altamonte Springs, and Oviedo override the county code inside those cities. Those codes are not in the Orange zoning or FLU knowledge base, so multifamily filters treat them as unknown rather than guessing. Lake does not publish assessed or taxable value on the open-data layer, and it has no situs city or qualified-sale flag. Osceola taxable value is the prior-roll `PrevTaxabl` field. `EstimatedT` is an estimate. `Q_U` and Sumter’s qualified-sale flag are stored and are not used to drop sales. St. Cloud is a city overlay, not an Osceola county extract. Winter Springs EnerGov is Seminole, not Osceola. Lake Mary AGOL is not a Seminole county substitute. Lake city FLU layers are city-only and are not joined. Sanford, Kissimmee, Clermont, and Lakeland stay gaps. Orange city overlays (Maitland, Winter Garden, Apopka, Winter Park, Orlando) are a separate change. Edgewood on maps.etcog.org is Edgewood, Texas.
