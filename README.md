...
6. **AADT.** Nearest count within 15 km. Florida uses FDOT 2025 segments in `data/fixtures/aadt-segments.geojson` for every footprint county. North Carolina, Georgia, South Carolina, Tennessee, Mississippi, Alabama, and Arkansas use `data/fixtures/aadt-state-dots.geojson` (published counts, not estimated) for footprint counties the layer actually covers. North Carolina is NCDOT `AADT_2022` (a string; blanks dropped). That FeatureServer is still HPMS 2022; newer NCDOT Connect shapefiles are not ingested. South Carolina is SCDOT 2025 points, field `FactoredAA`. Mississippi is the HDR AGOL republish of RCI layer `RC_AADT_2019`, field `ADT_21` (through 2021), not an MDOT-hosted service. The map overlay is still only Florida segments at or above 15,000. Include unknown AADT keeps parcels with no count in range. Outside Florida the drawer says Nearest AADT, not FDOT. A county with no published segment stays unknown.
...
| AADT | Florida: FDOT RCI `AADT` for 2025, every footprint county. NC, GA, SC, TN, MS, AL, and AR: that state's published DOT layer for footprint counties it actually covers. | FDOT RCI FeatureServer/0 `AADT` 2025; NCDOT `AADT_2022`; GDOT via UGA ITOS `AADT`; SCDOT `FactoredAA` 2025; TDOT `AADT`; Mississippi RCI `ADT_21`; ALDOT TDM `AADT` 2024; ARDOT `MostRecentADT` (actual stations) |
