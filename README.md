...
| AADT | Florida: FDOT 2025 historical counts. NC/GA/SC/TN/MS priority counties: that state's published DOT layer, filtered to the county. | FDOT historical TDA; NCDOT stations; GDOT via UGA ITOS; SCDOT 2025 points; TDOT traffic lines; Mississippi RCI `ADT_21` |
...
- AADT is the nearest published count within 15 km, not a local-road estimate. Florida uses the FDOT 2025 historical layer because `gis.fdot.gov` RCI FeatureServer/0 returned HTTP 500. Mecklenburg, Wake, Durham, Fulton, Cobb, DeKalb, Lowndes, Bibb, Charleston, Dorchester, Berkeley, Beaufort, Davidson, Hinds, Madison, and Rankin use that state’s DOT layer filtered to the county. Alabama, Arkansas, and every other county stay unknown. Outside Florida the drawer says Nearest AADT.
