...
**Site screening** (off until you turn a layer on): FEMA flood zones (with a base flood elevation only when NFHL publishes one, plus the community id and no invented CRS class), National Wetlands Inventory, public school ratings, Orange County, Charlotte-Mecklenburg, and DeKalb County attendance zones, and Orange County water, sewer, and electric service areas. Charlotte Water has no public polygon, so the drawer uses the city boundary only as a jurisdiction proxy and leaves unincorporated Mecklenburg unverified. Forty DeKalb County parcels in the Atlanta fixture join DCSD schools (GOSA 2025 CCRPI scores, not A–F letters), NFHL flood zones, and water, sewer, and electric. Two of those parcels sit in the City of Atlanta and still zone to Druid Hills DCSD; their water and sewer name Atlanta DWM as a jurisdiction proxy. The other 38 name DeKalb DWM the same way, because DeKalb publishes no service polygon. Gas stays unknown. Outside Orange County, electric falls back to HIFLD retail territories (Duke Energy Carolinas and EnergyUnited EMC in Mecklenburg; the DeKalb batch joins the cooperative when Georgia Power overlaps Snapping Shoals EMC or Walton EMC). The parcel drawer adds the mailing address already on the extract plus the county appraiser and, for entity owners, the state business search. It does not add phone numbers or emails. Coverage and refresh steps: [`docs/screening-layers.md`](docs/screening-layers.md).
