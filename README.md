...
Other MSAs use the same tile grid and the same zoom / area-lock / Show parcels gate. They load only while that market is selected. Refresh and the complete-vs-sample-vs-gap table: [`docs/market-parcels.md`](docs/market-parcels.md). The Pensacola shelf (Bay, Okaloosa, Walton, Escambia, Santa Rosa) uses county and city GIS instead of the statewide health-department extract: [`docs/panhandle-parcels.md`](docs/panhandle-parcels.md).
