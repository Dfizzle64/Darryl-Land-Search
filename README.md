...
| AADT | Florida: FDOT 2025 historical counts for every footprint county. NC, GA, SC, TN, MS, AL, and AR: that state's published DOT layer for footprint counties it actually covers. | FDOT historical TDA `AADT` 2025; NCDOT `AADT_2022`; GDOT via UGA ITOS `AADT`; SCDOT `FactoredAA` 2025; TDOT `AADT`; Mississippi RCI `ADT_21`; ALDOT TDM `AADT` 2024; ARDOT `MostRecentADT` (actual stations) |
...
FDOT_AADT_URL=https://services1.arcgis.com/O1JpcwDW8sjYuddV/arcgis/rest/services/Annual_Average_Daily_Traffic_Historical_TDA/FeatureServer/0/query
...
- AADT is the nearest published count within 15 km, not a local-road estimate. Florida uses the FDOT 2025 historical layer for every footprint county because `gis.fdot.gov` RCI FeatureServer/0 returned HTTP 500. North Carolina, South Carolina, Tennessee, Mississippi, Alabama, and Arkansas footprint counties use that state’s DOT layer. Georgia uses GDOT MapServer/21, which is sparse, so a county with no segment stays unknown. Outside Florida the drawer says Nearest AADT. This traffic join is separate from the South Carolina OZ 2.0 nomination work.
