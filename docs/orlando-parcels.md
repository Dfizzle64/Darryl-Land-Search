...
Median household income and FDOT AADT are not stored on the tiles. `npm run seed:signals` writes ACS 5-year 2020–2024 B19013 for every state with a live parcel extract (`data/fixtures/acs-b19013-tracts.json` and `data/fixtures/income-tracts.geojson`) and `data/fixtures/aadt-segments.geojson` (FDOT 2025, Florida). `/api/parcels` joins them at query time. A segment farther than 15 km is left unknown. That is a size choice, not a missing attribute: copying the same tract income onto every parcel would inflate the extract without adding a fact.
...
- Income and AADT are not stored on the parcel tiles. `/api/parcels` joins ACS 5-year 2020–2024 B19013 for every state with a parcel extract, and traffic counts at query time. Florida traffic is FDOT. Block-group income is still Orange County only. A set income minimum excludes parcels with no joined median unless Include unknown income is on.
