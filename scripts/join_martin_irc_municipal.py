#!/usr/bin/env python3
"""Stamp Martin and Indian River city zoning and future land use onto existing parcels.

Does not download county parcel polygons. Does not invent Opportunity Zones.
Ocean Breeze uses the Martin County zoning map only, and town-name stubs are
not stored. Sebastian future land use stays blank.

  python3 scripts/join_martin_irc_municipal.py
"""
...
CATALOG_PATH = ROOT / "data" / "martin-irc-municipal.json"
SUMMARY_PATH = ROOT / "data" / "fixtures" / "martin-irc-municipal-join.json"
COUNTY_DIR = ROOT / "data" / "fixtures" / "market-parcels" / "counties"
INDEX_PATH = ROOT / "data" / "fixtures" / "market-parcels" / "index.json"
META_PATHS = [
    ROOT / "data" / "fixtures" / "market-parcels" / "markets" / "vero-beach" / "meta.json",
    ROOT / "data" / "fixtures" / "market-parcels" / "markets" / "melbourne" / "meta.json",
]
CACHE = Path("/tmp/dls-martin-irc")
USER_AGENT = "darryl-land-search/martin-irc-municipal"
...
SEBASTIAN_FLU_GAP = "Sebastian publishes zoning only. City future land use was not invented."
OCEAN_STUB_GAP = (
    "Ocean Breeze has no city zoning service. The Martin County layer at this parcel is a town-name stub "
    "or NO DATA and was not stored."
)
...
def load_catalog() -> dict:
    ...
    if names != ["Stuart", "Indiantown", "Vero Beach", "Sebastian"]:
        raise SystemExit(f"Unexpected city list: {names}")
    if catalog["places"][-1].get("flu"):
        raise SystemExit("Sebastian future land use must stay a gap")
    if catalog["partials"][0].get("cityRest") is not None:
        raise SystemExit("Ocean Breeze must not invent a city service")
    fragments = catalog["blockedUrlFragments"]
    for place in catalog["places"]:
        for key in ("zoning", "flu"):
            layer = place.get(key)
            if layer and url_blocked(layer["url"], fragments):
...
def index_layer(catalog: dict, place: dict, layer: dict) -> tuple[dict[str, dict], SpatialIndex]:
    ...
    for feature in raw:
        ...
        if west < box[0] - 0.05 or south < box[1] - 0.05 or east > box[2] + 0.05 or north > box[3] + 0.05:
            continue
...
def point_code(url: str, field: str, x: float, y: float) -> str | None:
    data = fetch_json(
        query_url(url),
        {
    ...
    if not features:
        return None
    return clean((features[0].get("attributes") or {}).get(field))


def replace_gap(props: dict, replacement: str | None) -> None:
    gaps = [gap for gap in (props.get("dataGaps") or []) if gap not in {DOH_GAP, SEBASTIAN_FLU_GAP, OCEAN_STUB_GAP}]
...
def apply_place(catalog: dict, place: dict, group: list[dict]) -> tuple[dict, list[dict]]:
    ...
    for feature in group:
        ...
        if zone and zone.get("code"):
            ...
            props["municipal"] = {
                ...
                "fluGap": SEBASTIAN_FLU_GAP if place["fluStatus"] == "gap" else None,
            }
    ...
    if place["fluStatus"] == "city" and (zoning_joined < 1 or flu_joined < 1):
        raise SystemExit(f"{place['municipality']} did not join zoning and future land use")
    if place["fluStatus"] == "gap" and (flu_joined != 0 or zoning_joined < 1):
        raise SystemExit(f"Sebastian zoning/FLU result is not zoning-only: {zoning_joined} {flu_joined}")
    return {"parcels": len(group), "zoningJoined": zoning_joined, "fluJoined": flu_joined, "fluStatus": place["fluStatus"]}, overlays


def apply_ocean_breeze(catalog: dict, group: list[dict]) -> dict:
    partial = catalog["partials"][0]
    stubs = {item.upper() for item in catalog["stubCodes"]}
    zoning_joined = 0
    flu_joined = 0
    print(f"Join Ocean Breeze parcels {len(group)}", flush=True)
    for feature in group:
        props = feature["properties"]
        zone_code = None
        flu_code = None
        for x, y in candidate_points(feature):
            if zone_code is None:
                found = point_code(partial["zoning"]["url"], partial["zoning"]["code"], x, y)
                if found and found.upper() not in stubs:
                    zone_code = found
            if flu_code is None:
                found = point_code(partial["flu"]["url"], partial["flu"]["code"], x, y)
                if found and found.upper() not in stubs:
                    flu_code = found
            if zone_code and flu_code:
                break
        if zone_code:
            props["zoningCode"] = zone_code
            props["zoningDistrict"] = zone_code
            zoning_joined += 1
            props["municipal"] = {
                "placeId": "ocean-breeze",
                "placeName": "Ocean Breeze",
                "zoningLayer": partial["zoning"]["url"],
                "fluLayer": partial["flu"]["url"] if flu_code else None,
                "zoningLabel": None,
                "fluGap": None if flu_code else "Martin County future land use at this parcel was NO DATA or a stub and was not stored.",
            }
        if flu_code:
            props["flu"] = {
                "code": flu_code,
                ...
                "source": partial["flu"]["role"],
            }
            flu_joined += 1
        if not zone_code and not flu_code:
            replace_gap(props, OCEAN_STUB_GAP)
        else:
            replace_gap(props, None)
    print(f"  Ocean Breeze zoning {zoning_joined} flu {flu_joined}", flush=True)
    return {
        "parcels": len(group),
...
def stamp_county(catalog: dict, fips: str) -> dict:
    ...
    for place in catalog["places"]:
        ...
        stats[place["municipality"]] = place_stats
        overlays.extend(place_overlays)
    partial_stats = None
    if fips == "12085":
        group = []
        for situs in catalog["partials"][0]["situs"]:
            group.extend(by_situs.get(situs, []))
        partial_stats = apply_ocean_breeze(catalog, group)
        stats["Ocean Breeze"] = partial_stats
    for gap in catalog["gaps"]:
        if gap["fips"] != fips:
            continue
        for situs in gap["situs"]:
            for feature in by_situs.get(situs, []):
                props = feature["properties"]
                if props.get("zoningCode") or (props.get("flu") or {}).get("code"):
                    raise SystemExit(f"{gap['municipality']} received a zoning or future land use code")
...
def gap_lines(catalog: dict, fips: str, zoning: int, flu: int) -> list[str]:
    ...
    if fips == "12085":
        return [
            ...
            "Stuart zoning and future land use join on PCN when it matches, otherwise a point inside the parcel. Indiantown uses the village zoning and future land use layers.",
            "Ocean Breeze has no city service. Martin County zoning was checked only for those parcels. Town-name stubs and NO DATA were not stored.",
            "Sewall's Point and Jupiter Island stay blank. County labels JUPITER ISLAND and SEWALLS POINT are not districts.",
            "Rejected: Ocoee ParcelsFLUZoning, Palm Beach County zoning, and ClearviewGeographic stacks.",
        ]
    return [
        f"Florida DOH 5–150 acre parcels ({county['source']}, {zoning} zoning codes and {flu} future land use codes). County parcels were not re-downloaded.",
        "Vero Beach zoning is ZoningDistricts and future land use is ZoningFutureLandUse. Those code lists are not interchangeable. The token-gated COVB folder is not called.",
        "Sebastian zoning is COS_Zoning. Sebastian future land use is still a gap.",
        "Fellsmere, Indian River Shores, and Orchid stay blank. No city FeatureServer was invented.",
    ]


def patch_lists(summaries: list[dict]) -> None:
    by_fips = {item["fips"]: item for item in summaries}

    def walk(node: object) -> None:
        if isinstance(node, dict):
            fips = node.get("fips")
            summary = by_fips.get(fips)
            if summary and node.get("name") == summary["name"]:
                node["gaps"] = gap_lines(json.loads(CATALOG_PATH.read_text()), fips, summary["zoningJoinedCount"], summary["fluJoinedCount"])
    ...
    for path in [INDEX_PATH, *META_PATHS]:
...
def main() -> None:
    ...
    summaries = [stamp_county(catalog, fips) for fips in ("12085", "12061")]
    patch_lists(summaries)
    ...
    print("Martin and Indian River city overlays joined onto existing county parcels.", flush=True)
