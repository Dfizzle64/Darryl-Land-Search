...
Sources (Rev. Proc. 2026-14, approximate 90-minute county rings):
  ...
  data/oz2-other-msas-eligible.csv             — rural + non-rural, 16 smaller MSAs
...
def assert_other_splits(rows: list[dict]) -> None:
    ...
    if len(rural_rows) != 515 or len(urban_rows) != 928:
...
def main() -> None:
    ...
    assert_counts(other_rows, {market: total for market, (total, _r, _u) in OTHER_EXPECTED.items()}, "other-msas", 1398)
    ...
    if len(unique) != 2342:
        raise RuntimeError(f"Expected 2342 combined unique GEOIDs, found {len(unique)}")
    if offline:
    ...
    if len(features) != 2342:
        raise RuntimeError(f"Expected 2342 polygons, built {len(features)}")
