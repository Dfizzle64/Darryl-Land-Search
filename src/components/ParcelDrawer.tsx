...
export function ParcelDrawer({
  ...
  const knoxCounty = properties.countyFips === "47093";
  const zoningEmpty = knoxCounty
    ? "No zoning polygon joined at the centroid"
    : properties.countyFips === "12095"
      ? "Not on the OCPA parcel"
      : "Not in this county's public parcel extract";
  const fluEmpty = knoxCounty
    ? properties.municipality === "Unincorporated Knox County"
      ? "No Advance Knox place type at the centroid"
      : "No public future land use layer for this municipality"
  ...
  const fluLine = properties.flu?.code
    ...
    : null;
  const stateLabel =
    properties.state === "Tennessee" ? "TN" : properties.state === "Florida" || !properties.state ? "FL" : properties.state;
  const placeLine =
    [properties.situsCity, properties.situsZip].filter(Boolean).join(" ") ||
    (properties.countyName ? `${properties.countyName} County, ${stateLabel}` : "Florida");
  ...
  return (
    <aside className={pane ? "drawer-scroll h-full overflow-y-auto bg-ink-900 p-5" : "drawer-scroll absolute inset-x-0 bottom-0 z-20 max-h-[70vh] overflow-y-auto rounded-t-3xl border border-white/10 bg-ink-900 p-5 shadow-2xl lg:static lg:z-0 lg:max-h-none lg:w-[24rem] lg:shrink-0 lg:rounded-none lg:border-l lg:border-t-0 lg:shadow-none"}>
      ...
      <dl className="mt-5 grid grid-cols-2 gap-4">
        ...
        <Field label={knoxCounty ? "Appraised total" : "Market value"} value={formatUsd(properties.tax.marketValue)} />
        {properties.tax.appraisedLand !== undefined ? (
          <Field label="Appraised land" value={formatUsd(properties.tax.appraisedLand)} />
        ) : null}
        {properties.tax.appraisedBuilding !== undefined ? (
          <Field label="Appraised building" value={formatUsd(properties.tax.appraisedBuilding)} />
        ) : null}
        ...
        {florida && entity && properties.ownerName ? (
          ...
          <p className="text-ink-300">
            {florida
              ...
              : "Sunbiz is the Florida entity search and is not used outside Florida."}
          </p>
        )}
