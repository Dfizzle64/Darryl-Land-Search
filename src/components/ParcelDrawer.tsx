...
export function ParcelDrawer({
  ...
  const zoningLine = properties.zoningCode
    ? [properties.zoningCode, properties.jurisdictionPrefix].filter(Boolean).join(" · ")
    : null;
  ...
  const placeLine =
    ...
    (properties.countyName ? `${properties.countyName} County, ${stateAbbr}` : properties.state || "Florida");
