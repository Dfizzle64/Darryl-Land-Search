...
export async function screeningAtPoint(lon: number, lat: number, parcelId?: string | null): Promise<ScreeningPoint> {
  const dekalb = dekalbBatchParcel(parcelId);
  const dekalbOverlay = dekalb ? dekalbScreeningOverlay(dekalb, lon, lat) : null;
  const [flood, wetland, water, sewer, power, gas, schools, tract] = await Promise.allSettled([
    dekalbOverlay ? Promise.resolve(dekalbOverlay.flood) : floodAtPoint(lon, lat),
    wetlandAtPoint(lon, lat),
    dekalbOverlay ? Promise.resolve(dekalbOverlay.utilities[0]) : utilityAtPoint("water", lon, lat),
    dekalbOverlay ? Promise.resolve(dekalbOverlay.utilities[1]) : utilityAtPoint("sewer", lon, lat),
    dekalbOverlay ? Promise.resolve(dekalbOverlay.utilities[2]) : utilityAtPoint("power", lon, lat),
    dekalbOverlay ? Promise.resolve(dekalbOverlay.utilities[3]) : utilityAtPoint("gas", lon, lat),
    dekalbOverlay ? Promise.resolve({ schools: dekalbOverlay.schools, note: dekalbOverlay.schoolsNote }) : schoolsNear(lon, lat),
    dekalbOverlay ? Promise.resolve(null) : meckTractAtPoint(lon, lat),
  ]);
  const schoolResult =
