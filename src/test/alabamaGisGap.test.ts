...
describe("Alabama GIS re-chase extracts", () => {
  it("keeps Walker, Decatur AL, Washington AL, and Escambia AL as gaps", () => {
    for (const fips of ["01127", "01103", "01129", "01053"]) {
      const row = countyMeta(fips);
      expect(row.featureCount).toBe(0);
      expect(row.coverage).toBe("gap");
    }
    const morgan = countyMeta("01103").gaps.join(" ");
    expect(morgan.toLowerCase()).not.toContain("decatur, illinois");
    expect(morgan.toLowerCase()).not.toContain("decatur il");
  });
